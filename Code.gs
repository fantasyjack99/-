 ...
 * 取得本週已完成的審核記錄
 */
function getInspectionDates(weekNumber) {
  // 取得本週的巡檢日期
...
function showApprovalPage(weekNumber, level) {
  ...
  const weekData = getWeekInspectionDataForApproval(weekNumber);
  const inspectionDates = getInspectionDates(weekNumber);
  
  // 建立巡檢日期列表
  let inspectionDatesHtml = '';
  for (const day of inspectionDates) {
    const statusIcon = day.hasAbnormal ? '⚠️' : '✅';
    const statusText = day.hasAbnormal ? '有異常' : '正常';
    inspectionDatesHtml += `<div style="padding:8px 12px;background:#e3f2fd;border-radius:6px;margin-bottom:8px;color:#1565c0;font-size:13px;">${statusIcon} ${day.date} (${day.inspector}) - ${statusText}</div>`;
  }
  if (inspectionDatesHtml) {
    inspectionDatesHtml = `<div class="section-title">📜 巡檢記錄</div>` + inspectionDatesHtml;
  }
